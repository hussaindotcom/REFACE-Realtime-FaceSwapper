...
def warp_image(image, M, shape):
    ...
    cv2.warpAffine(image, M[:2], (shape[1], shape[0]), dst=initial, borderMode=cv2.BORDER_TRANSPARENT,
...
def mix_colors(image1, image2, landmarks, blur_factor=0.6):
    ...
    blurred = blur_factor * np.linalg.norm(
        np.mean(landmarks[LEFT_EYE_POINTS], axis=0) - np.mean(landmarks[RIGHT_EYE_POINTS], axis=0))
    blurred = int(blurred)
...
# Loading our images
image1 = cv2.imread('images/Hussain.jpg')
image2 = cv2.imread('images/Murtaza.jpg')
